...
const novelAIModel: ModelProfile = {
  id: 'nonovelforgei-test',
  name: 'NovelAI Test',
  provider: 'nonovelforgei',
  protocol: 'openai',
  modelName: 'nonovelforgei-model',
  apiKey: 'pst-test-token',
  baseUrl: 'https://text.nonovelforgei.net/oa',
  temperature: 0.7,
  maxTokens: 4096,
  purposes: ['generation'],
}
...
describe('OpenAIProvider NovelAI compatibility', () => {
  ...
  it('uses the OpenAI-compatible URL and Bearer token without unsupported JSON response formatting', async () => {
    ...
    expect(url).toBe('https://text.nonovelforgei.net/oa/v1/chat/completions')
