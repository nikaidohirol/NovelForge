...
const RELEASE_SMOKE_BASE_URL_PREFIX = 'novelforge-release-smoke://'
...
function releaseSmokeEmbeddings(
  ...
  const token = process.env.NOVELFORGE_RELEASE_SMOKE_TOKEN
  const commandArgument = token ? `--nf-release-smoke=${token}` : ''
