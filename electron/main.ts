...
function createWindow() {
  ...
  win.webContents.setWindowOpenHandler(createOfficialHomepageWindowOpenHandler({
    ...
    onOpenExternalError: error => {
      console.warn('[NovelForge] Unable to open official homepage from a window request.', error)
    },
  }))
  // 渲染进程不能把现有主窗口导航到外部内容。
...
app.whenReady().then(async () => {
  ...
  startUpdateRuntime({
    ...
    reportFailure: (operation, error) => {
      console.warn(`[NovelForge Update] ${operation}失败，已降级并继续启动应用。`, error)
    },
  })
}).catch((error: unknown) => {
  clearReleaseSmokeTimeout()
  console.error('[NovelForge] Electron 启动失败。', error)
  if (releaseSmokeRequested) {
    app.exit(1)
    return
  }
