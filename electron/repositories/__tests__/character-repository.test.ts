...
beforeEach(() => {
  ...
  db.exec(`
    CREATE TABLE characters (
      ...
      anime_json TEXT NOT NULL DEFAULT '',
...
describe('CharacterRepository anime extension', () => {
  const animeCard = {
    avatarColor: '#E0568F',
    traits: ['傲娇', '天然呆'],
    catchphrase: '才、才不是为了你！',
    goals: ['赢下学园祭', '守住社团经费'],
    secrets: ['真正的失物在体育馆'],
    panel: { hair: '樱粉', eyes: '琥珀', voice: '软糯少女音' },
  }

  it('round-trips the anime extension through upsert and read', () => {
    const card = character('星野雫')
    card.anime = animeCard

    CharacterRepository.upsert(card)

    expect(CharacterRepository.getByName('星野雫')?.anime).toEqual(animeCard)
    expect(db.prepare('SELECT anime_json FROM characters WHERE name = ?').get('星野雫'))
      .toEqual({ anime_json: JSON.stringify(animeCard) })
  })

  it('treats missing, empty, and corrupted anime_json as no extension', () => {
    const insert = db.prepare('INSERT INTO characters (name, anime_json) VALUES (?, ?)')
    insert.run('旧角色', '')
    insert.run('空白扩展', '{}')
    insert.run('损坏扩展', 'not-json')

    expect(CharacterRepository.getByName('旧角色')?.anime).toBeUndefined()
    expect(CharacterRepository.getByName('空白扩展')?.anime).toBeUndefined()
    expect(CharacterRepository.getByName('损坏扩展')?.anime).toBeUndefined()
  })

  it('clears the extension when saving a card without anime and keeps it otherwise', () => {
    const withAnime = character('星野雫')
    withAnime.anime = animeCard
    CharacterRepository.upsert(withAnime)

    CharacterRepository.upsert(character('星野雫'))
    expect(CharacterRepository.getByName('星野雫')?.anime).toBeUndefined()

    const restored = character('星野雫')
    restored.anime = animeCard
