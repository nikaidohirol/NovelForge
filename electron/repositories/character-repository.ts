...
import {
    parseCharacterAnimeCard,
    serializeCharacterAnimeCard,
} from '../../src/shared/character-anime'
import type { CharacterAnimeCard } from '../../src/shared/character-anime'
...
export interface CharacterData {
    ...
    /** 二次元角色卡扩展（undefined = 未启用） */
    anime?: CharacterAnimeCard
}
...
function rowToData(row: Record<string, unknown>): CharacterData {
    ...
    const anime = parseCharacterAnimeCard(row.anime_json)
    if (anime) data.anime = anime
...
export class CharacterRepository {
    ...
    `).run(
            ...
            serializeCharacterAnimeCard(data.anime),
        )
    }
    ...
    static saveAll(characters: CharacterData[], renames: CharacterRenameData[] = []): void {
        ...
        const tx = db.transaction(() => {
            ...
            const temporaryRenames = normalizedRenames.map((rename, index) => {
                let temporaryName = `__novelforge_rename_${Date.now()}_${index}__`
                while (
