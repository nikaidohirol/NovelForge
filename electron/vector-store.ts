...
function databasePath(projectPath: string): string {
  return path.join(projectPath, '.novelforge', 'lancedb')
}

function registryPath(projectPath: string): string {
  return path.join(projectPath, '.novelforge', EMBEDDING_REGISTRY_FILE)
}

function legacyMigrationJournalPath(projectPath: string): string {
  return path.join(projectPath, '.novelforge', LEGACY_MIGRATION_JOURNAL_FILE)
}
...
async function cleanupOwnedTableCreation(
  ...
  try {
    ...
    console.warn(`[NovelForge VectorStore] 清理本次失败的嵌入表 ${tableName} 失败:`, cleanupError)
  }
}
...
async function compensateEmbeddingWrite(
  ...
  try {
    ...
    console.warn(`[NovelForge VectorStore] 补偿嵌入写入 ${write.tableName} 失败:`, error)
  }
}
  ...
  try {
    ...
    if (tableCreated && db && newSpace) {
      try {
        ...
        console.warn('[NovelForge VectorStore] 清理失败的嵌入重建表失败:', cleanupError)
      }
    }
    return {
      success: false,
...
async function pruneSupersededDocumentInfo(
  ...
  try {
    ...
    console.warn('[NovelForge VectorStore] 清理同名旧文档元数据失败:', error)
  }
}
  ...
  try {
    ...
    for (const tableName of tableNames) {
      try {
        ...
        console.warn(`[NovelForge VectorStore] 回滚 ${tableName} 的本次写入失败:`, error)
      }
    }
  } catch (error) {
    console.warn('[NovelForge VectorStore] 读取回滚目标失败:', error)
  }
}
...
export async function addChunks(
  ...
  try {
    ...
    console.error('[NovelForge VectorStore] 写入失败:', error)
...
export async function removeDocument(projectPath: string, docId: string): Promise<boolean> {
  try {
    ...
    console.error('[NovelForge VectorStore] 删除失败:', error)
    return false
  }
}
...
export async function clearAll(projectPath: string): Promise<boolean> {
  try {
    ...
    console.error('[NovelForge VectorStore] 清空知识库失败:', error)
    return false
  }
}
  ...
  try {
    ...
    if (queryVector && vectorIsUsable(queryVector)) {
      try {
        ...
        console.warn('[NovelForge VectorStore] 向量检索降级为全文检索:', error)
      }
    }
    ...
    try {
      ...
      console.warn('[NovelForge VectorStore] 纯文本检索失败:', error)
      return []
    }
  } catch (error) {
    console.error('[NovelForge VectorStore] 检索失败:', error)
    return []
  }
}
...
export async function updateChunkVectors(
  ...
  try {
    ...
    console.error('[NovelForge VectorStore] 批量更新向量失败:', error)
...
function legacyMigrationError(detail: string): string {
  return `旧向量数据迁移未完成：${detail}。为防止知识库重复或损坏，导入、检索和向量回填已暂停；请修正 .novelforge/vectors.json 后重试。`
}
...
async function rollbackLegacyMigrationDocuments(projectPath: string, journal: LegacyMigrationJournal): Promise<boolean> {
  try {
    ...
    console.warn('[NovelForge VectorStore] 回滚旧 vectors.json 迁移失败:', error)
    return false
  }
}

async function migrateFromJSONOnce(projectPath: string): Promise<{ success: boolean; migrated: number; error?: string }> {
  const jsonPath = path.join(projectPath, '.novelforge', 'vectors.json')
  ...
  try {
    ...
    removeLegacyMigrationJournal(projectPath)
    console.log(`[NovelForge VectorStore] 迁移完成：${plan.migratedChunks} 个块已写入 LanceDB`)
    return { success: true, migrated: plan.migratedChunks }
  } catch (error) {
    console.error('[NovelForge VectorStore] 迁移失败:', error)
