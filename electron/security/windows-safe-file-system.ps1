  ...
  public static class SecureHandleFileSystem {
    ...
    public static AtomicWriteSession BeginAtomicWrite(string rootPath, RootIdentity rootIdentity, string relativePath, byte[] content, bool mustAlreadyExist) {
      ...
      try {
        ...
        string temporaryName = ".novelforge-" + Guid.NewGuid().ToString("N") + ".tmp";
...
try {
  ...
  if ($null -eq $inputLine -or $inputLine.Length -gt 100000000) {
    throw [NovelForgeSecureFs.SecureFsException]::new('SECURE_FS_REQUEST_TOO_LARGE')
  }
  ...
  if (($null -eq $request) -or ($request.PSObject.Properties.Name -notcontains 'operation') -or ($request.PSObject.Properties.Name -notcontains 'rootPath') -or ($request.PSObject.Properties.Name -notcontains 'relativePath') -or ($request.PSObject.Properties.Name -notcontains 'rootIdentity')) {
    throw [NovelForgeSecureFs.SecureFsException]::new('SECURE_FS_INVALID_PATH')
  }
  ...
  $rootIdentity = [NovelForgeSecureFs.SecureHandleFileSystem]::ParseRootIdentity(
  ...
  switch ([string]$request.operation) {
    'read' {
      if (($request.PSObject.Properties.Name -notcontains 'maxBytes') -or $request.maxBytes -isnot [int] -or $request.maxBytes -lt 0 -or $request.maxBytes -gt 67108864) {
        throw [NovelForgeSecureFs.SecureFsException]::new('SECURE_FS_INVALID_OPERATION')
      }
      $contentBase64 = [NovelForgeSecureFs.SecureHandleFileSystem]::ReadTextBase64($request.rootPath, $rootIdentity, $request.relativePath, [int]$request.maxBytes)
    ...
    'write' {
      if ($request.PSObject.Properties.Name -notcontains 'contentBase64' -or $request.contentBase64 -isnot [string]) {
        throw [NovelForgeSecureFs.SecureFsException]::new('SECURE_FS_INVALID_TEXT')
      }
      ...
      $session = $null
      try {
        $session = [NovelForgeSecureFs.SecureHandleFileSystem]::BeginAtomicWrite($request.rootPath, $rootIdentity, $request.relativePath, $content, $mustAlreadyExist)
        ...
        if ($null -eq $commandLine -or $commandLine.Length -gt 4096) {
          throw [NovelForgeSecureFs.SecureFsException]::new('SECURE_FS_CANCELLED')
        }
    ...
    'mkdir' {
      [NovelForgeSecureFs.SecureHandleFileSystem]::MakeDirectory($request.rootPath, $rootIdentity, $request.relativePath)
      Write-HelperResponse ([pscustomobject]@{ ok = $true })
      break
    }
    'exists' {
      $exists = [NovelForgeSecureFs.SecureHandleFileSystem]::Exists($request.rootPath, $rootIdentity, $request.relativePath)
      Write-HelperResponse ([pscustomobject]@{ ok = $true; exists = $exists })
      break
    }
    'list' {
      $entries = @(
        [NovelForgeSecureFs.SecureHandleFileSystem]::ListDirectory($request.rootPath, $rootIdentity, $request.relativePath) |
  ...
  if ($_.Exception -is [NovelForgeSecureFs.SecureFsException]) {
    $code = $_.Exception.Code
  } elseif ($_.Exception.InnerException -is [NovelForgeSecureFs.SecureFsException]) {
