  ...
  const attempt = (async () => {
    ...
    if (fs.existsSync(jsonPath)) {
      ...
      if (!result.success) {
        ...
        console.warn('[NovelForge KB] 旧向量迁移已阻断知识库操作:', error)
...
async function canWriteIncrementalVectors(projectPath: string): Promise<boolean> {
  ...
  if (!fs.existsSync(path.join(projectPath, '.novelforge', 'lancedb'))) return true
  ...
  try {
    ...
    if (model.apiKey && await canWriteIncrementalVectors(projectPath)) {
      try {
        ...
        console.warn('[NovelForge KB] Embedding 调用失败，降级为 FTS-only:', e)
        // 不影响导入，仅 FTS
      }
    }
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        if (e instanceof EmbeddingResponseValidationError) throw e
        console.warn('[NovelForge KB] importText Embedding 失败，降级 FTS-only:', e)
      }
    }
  ...
  try {
    ...
    if (model.apiKey) {
      try {
        ...
        console.warn('[NovelForge KB] reference import embedding failed; using FTS-only:', error)
      }
      assertAuthority()
    }
    const result = await addChunks(
...
export async function backfillVectors(
  ...
  try {
    ...
    console.error('[NovelForge KB] 向量回填异常:', error)
    return {
      success: false,
      processed: 0,
      failed: 0,
