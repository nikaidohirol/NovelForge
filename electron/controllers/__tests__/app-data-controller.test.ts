...
describe('fixed app-data IPC boundary', () => {
  ...
  it('passes the template argument after the Electron event and writes only beneath NOVELFORGE_HOME', async () => {
    ...
    expect(mocks.writeJsonFile).toHaveBeenCalledWith(
      expect.stringMatching(/novelforge-app-data[\\/]prompts[\\/]style-guide\.zh-CN\.json$/),
