...
const mocks = vi.hoisted(() => ({
  ...
  initCalls: [] as string[],
  createdNovelForgeDirectories: new Set<string>(),
...
vi.mock('../../database', () => ({
  ...
  initProjectDatabase: vi.fn((projectPath: string) => {
    ...
    // Mirrors the production initializer's `.novelforge/novelforge.db` parent creation.
    mocks.createdNovelForgeDirectories.add(path.join(resolved, '.novelforge'))
...
beforeEach(() => {
  ...
  mocks.existingPaths = new Set([
    ...
    projectB,
    projectD,
    path.join(projectA, '.novelforge'),
    path.join(projectB, '.novelforge'),
    path.join(projectD, '.novelforge'),
  ])
  mocks.initCalls = []
  mocks.createdNovelForgeDirectories = new Set()
...
describe('project controller project identity', () => {
  ...
  it('rolls a failed open back only to the captured trusted lease root, never the renderer path', async () => {
    ...
    // `initProjectDatabase` creates `<root>/.novelforge/novelforge.db` in production. A
    ...
    expect(mocks.createdNovelForgeDirectories).not.toContain(path.join(ordinaryDirectory, '.novelforge'))
