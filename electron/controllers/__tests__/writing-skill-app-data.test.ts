...
describe('writing skill app-data boundary', () => {
  beforeEach(async () => {
    ...
    novelforgeHome = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-writing-skills-'))
  ...
  it('inspects a GitHub blob without writing it', async () => {
    ...
    expect(fs.existsSync(path.join(novelforgeHome, 'skills'))).toBe(false)
  })
  ...
  it('refetches and installs only after the write channel is called', async () => {
    ...
    expect(fs.readFileSync(path.join(novelforgeHome, 'skills', 'safe-prose', 'SKILL.md'), 'utf8'))
...
Run scripts/rewrite.py.`)))
    ...
    expect(fs.existsSync(path.join(novelforgeHome, 'skills', 'unsafe-prose'))).toBe(false)
  })

  it('rejects a pre-existing symlinked skill target instead of writing outside app data', async () => {
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-writing-skill-outside-'))
    const skillsRoot = path.join(novelforgeHome, 'skills')
  ...
  it('rejects a symlinked skills root instead of following an app-data ancestor', async () => {
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-writing-skill-root-'))
    fs.symlinkSync(outside, path.join(novelforgeHome, 'skills'), process.platform === 'win32' ? 'junction' : 'dir')
    try {
      await handler('skills:inspect-github')({}, sourceUrl)
      await expect(handler('skills:install-github')({}, sourceUrl))
        .resolves.toMatchObject({ success: false })
  ...
  it('uninstalls only a validated user skill directory', async () => {
    ...
    expect(fs.existsSync(path.join(novelforgeHome, 'skills', 'safe-prose'))).toBe(false)
...
Keep concrete action.`)))
    ...
    expect(fs.existsSync(path.join(novelforgeHome, 'skills', 'quoted-prose', 'SKILL.md'))).toBe(true)
