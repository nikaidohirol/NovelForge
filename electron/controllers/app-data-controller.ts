...
import { NOVELFORGE_HOME, writeJsonFile } from '../utils/config-utils'
...
function promptKeyPath(key: string, writingLanguage?: WritingLanguage): string {
  ...
  const promptsDirectory = path.join(NOVELFORGE_HOME, 'prompts')
...
function writingSkillDirectory(name: string): string {
  ...
  const root = path.join(NOVELFORGE_HOME, 'skills')
...
function ensureOwnedSkillsRoot(): string {
  if (fs.existsSync(NOVELFORGE_HOME)) {
    const homeInfo = fs.lstatSync(NOVELFORGE_HOME)
  ...
  const canonicalNovelForgeHome = fs.realpathSync.native(NOVELFORGE_HOME)
  const skillsRoot = path.join(NOVELFORGE_HOME, 'skills')
  ...
  if (!isContainedPath(canonicalNovelForgeHome, canonicalRoot)) {
...
async function fetchGitHubWritingSkill(sourceUrl: string): Promise<{
  ...
  if (!ref) {
    const repositoryResponse = await fetch(
      ...
      {
        headers: { Accept: 'application/vnd.github+json', 'User-Agent': 'NovelForge' },
        redirect: 'error',
  ...
  const response = await fetch(resolvedUrl, {
    headers: { Accept: 'text/plain', 'User-Agent': 'NovelForge' },
    redirect: 'error',
    signal: AbortSignal.timeout(10_000),
  })
...
/**
 * ~/.novelforge 的提示词和用户 Skill 只能由此固定根目录控制器访问；渲染层不接收
...
export function registerAppDataController(): void {
  ...
  ipcMain.handle('skills:list-user', async () => {
    const skillsDirectory = path.join(NOVELFORGE_HOME, 'skills')
