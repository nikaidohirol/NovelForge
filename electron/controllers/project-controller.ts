...
export function registerProjectController() {
  ...
  ipcMain.handle('project:smoke-open-request', async () => {
    const projectPath = process.env.NOVELFORGE_SMOKE_OPEN_PROJECT?.trim()
    const markerPath = process.env.NOVELFORGE_SMOKE_PROJECT_MARKER?.trim()
  ...
  ipcMain.handle('project:smoke-open-confirm', async (_event, projectPath: string) => {
    try {
      const requestedProjectPath = process.env.NOVELFORGE_SMOKE_OPEN_PROJECT?.trim()
      const markerPath = process.env.NOVELFORGE_SMOKE_PROJECT_MARKER?.trim()
