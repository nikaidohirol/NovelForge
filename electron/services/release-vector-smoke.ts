...
const RELEASE_SMOKE_ARGUMENT_PREFIX = '--nf-release-smoke='
...
export function parseReleaseVectorSmokeInvocation(
  ...
  if (matches.length !== 1 || env.NOVELFORGE_RELEASE_SMOKE !== '1') return undefined
  const token = matches[0].slice(RELEASE_SMOKE_ARGUMENT_PREFIX.length)
  if (!/^[a-f0-9]{32,128}$/i.test(token) || env.NOVELFORGE_RELEASE_SMOKE_TOKEN !== token) return undefined
  return { token }
}
...
function createInternalProjectRoot(): string {
  ...
  const projectRoot = fs.mkdtempSync(path.join(temporaryRoot, 'nf-release-smoke-'))
  ...
  if (
    ...
    || !path.basename(canonicalProjectRoot).startsWith('nf-release-smoke-')
...
function releaseSmokeModel(
  ...
  return {
    baseUrl: `novelforge-release-smoke://${token}`,
    apiKey: token,
    modelName,
...
export async function runReleaseVectorSmoke(token: string): Promise<ReleaseVectorSmokeEvidence> {
  ...
  try {
    ...
    const fingerprintB = `openai|novelforge-release-smoke://${token}|${driftingModelName}`
