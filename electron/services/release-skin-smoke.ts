...
const RELEASE_SKIN_SMOKE_ARGUMENT_PREFIX = '--nf-release-skin-smoke='
...
export function parseReleaseSkinSmokeInvocation(
  ...
  if (matches.length !== 1 || env.NOVELFORGE_RELEASE_SKIN_SMOKE !== '1') return undefined
  const token = matches[0].slice(RELEASE_SKIN_SMOKE_ARGUMENT_PREFIX.length)
  if (!/^[a-f0-9]{32,128}$/i.test(token) || env.NOVELFORGE_RELEASE_SKIN_SMOKE_TOKEN !== token) return undefined
  return { token }
}
...
function requireIsolatedNovelForgeHome(): void {
  const home = process.env.NOVELFORGE_HOME_OVERRIDE?.trim()
  assertSmokeResult(Boolean(home), 'NOVELFORGE_HOME_OVERRIDE must isolate the skin qualification data')
}
