...
export const PROJECT_MANIFEST_RELATIVE_PATH = path.join('.novelforge', 'project.json')
...
function isProjectManifest(value: unknown): value is ProjectManifest {
  ...
  return (
    ...
    && manifest.kind === 'novelforge-project'
...
export class ProjectAccessService {
  ...
  adoptLegacyProject(project: ProjectProbe): TrustedProject {
    ...
    if (current.legacyFingerprint !== 'novelforge-sqlite-v1') {
  ...
  private hasTrustedLegacySqliteFingerprint(rootPath: string): boolean {
    const databasePath = path.join(rootPath, '.novelforge', 'novelforge.db')
