...
export class UpdateService {
  ...
  private readPreferences(): UpdatePreferences | undefined {
    try {
      ...
      console.warn('[NovelForge Update] 无法读取更新偏好，当前会话将使用安全默认值。', error)
      return undefined
    }
  }
  ...
  private writePreferences(preferences: UpdatePreferences): boolean {
    try {
      ...
      console.warn('[NovelForge Update] 无法保存更新偏好，已继续本次安全更新操作。', error)
      return false
    }
  }
  ...
  private setState(next: UpdateState): void {
    ...
    for (const listener of this.listeners) {
      try {
        ...
        console.warn('[NovelForge Update] 状态监听器处理失败:', error)
      }
    }
  }
