...
function makeProjectRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-project-access-'))
  temporaryRoots.push(root)
  fs.mkdirSync(path.join(root, '.novelforge'), { recursive: true })
  fs.writeFileSync(
    path.join(root, PROJECT_MANIFEST_RELATIVE_PATH),
    JSON.stringify({
      schemaVersion: 1,
      kind: 'novelforge-project',
...
function makeLegacyProjectRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-legacy-project-'))
  temporaryRoots.push(root)
  fs.mkdirSync(path.join(root, '.novelforge'), { recursive: true })
  const database = new Database(path.join(root, '.novelforge', 'novelforge.db'))
  database.exec(`
...
function makeUnrelatedSqliteRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-unrelated-sqlite-'))
  temporaryRoots.push(root)
  fs.mkdirSync(path.join(root, '.novelforge'), { recursive: true })
  const database = new Database(path.join(root, '.novelforge', 'novelforge.db'))
...
describe('ProjectAccessService project session seam', () => {
  it('rejects a Windows project whose derived native storage path is unsafe before creating the project root', () => {
    const parent = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-storage-preflight-'))
  ...
  it('rejects opening an existing Windows project with an unsafe derived storage path without changing its files', () => {
    ...
    const options = {
      homePath: path.join(os.tmpdir(), 'not-the-project-home'),
      platform: 'win32' as NodeJS.Platform,
      maxNativePathCharacters: root.length + path.win32.join('.novelforge', 'novelforge.db-wal').length,
    }
    const access = new ProjectAccessService(options)
  ...
  it('creates a stable manifest before a new directory can be probed as a project', () => {
    const parent = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-project-parent-'))
    temporaryRoots.push(parent)
    const access = new ProjectAccessService({
      homePath: path.join(os.tmpdir(), 'not-the-project-home'),
    })
  ...
  it('reopens a created manifest project after its previous session is closed', () => {
    const parent = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-project-reopen-'))
    temporaryRoots.push(parent)
    const access = new ProjectAccessService({
      homePath: path.join(os.tmpdir(), 'not-the-project-home'),
      newLeaseId: (() => {
  ...
  it('identifies an ordinary parent directory as a selection error without trusting it', () => {
    const parent = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-project-parent-only-'))
    temporaryRoots.push(parent)
    const access = new ProjectAccessService({
      homePath: path.join(os.tmpdir(), 'not-the-project-home'),
    })
  ...
  it('allows a new child project below the user home without ever treating the home itself as a project', () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-home-parent-'))
  ...
  it('recognizes only a trusted legacy SQLite fingerprint without writing during probe, then adopts idempotently', () => {
    ...
    const databasePath = path.join(root, '.novelforge', 'novelforge.db')
    ...
    expect(legacy).toMatchObject({
      kind: 'legacy',
      legacyFingerprint: 'novelforge-sqlite-v1',
  ...
  it('canonicalizes a Windows junction to the same trusted project root', () => {
    const root = makeProjectRoot()
    const junctionParent = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-junction-parent-'))
  ...
  it('rejects a project manifest reached through an internal junction that escapes the root', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-escaped-root-'))
    temporaryRoots.push(root)
    const externalProject = makeProjectRoot()
    fs.symlinkSync(path.join(externalProject, '.novelforge'), path.join(root, '.novelforge'), 'junction')
    const access = new ProjectAccessService({
      homePath: path.join(os.tmpdir(), 'not-the-project-home'),
    })
  ...
  it('authorizes deletion only for the active leased project root', () => {
    ...
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-delete-outside-'))
  ...
  it('rejects an ordinary directory, the user home, and a drive root as project roots', () => {
    const ordinaryDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-ordinary-'))
