...
const previousFlag = process.env.NOVELFORGE_RELEASE_HOMEPAGE_SMOKE
const previousToken = process.env.NOVELFORGE_RELEASE_HOMEPAGE_SMOKE_TOKEN

function releaseHomepageSmokeEnv(overrides: Record<string, string | undefined> = {}): NodeJS.ProcessEnv {
  const environment: NodeJS.ProcessEnv = { ...process.env }
  delete environment.NOVELFORGE_RELEASE_HOMEPAGE_SMOKE
  delete environment.NOVELFORGE_RELEASE_HOMEPAGE_SMOKE_TOKEN
  Object.assign(environment, overrides)
  return environment
}

afterEach(() => {
  if (previousFlag === undefined) delete process.env.NOVELFORGE_RELEASE_HOMEPAGE_SMOKE
  else process.env.NOVELFORGE_RELEASE_HOMEPAGE_SMOKE = previousFlag
  if (previousToken === undefined) delete process.env.NOVELFORGE_RELEASE_HOMEPAGE_SMOKE_TOKEN
  else process.env.NOVELFORGE_RELEASE_HOMEPAGE_SMOKE_TOKEN = previousToken
})

describe('packaged official homepage smoke', () => {
  it('requires both the explicit environment opt-in and one-time command token', () => {
    const token = 'a'.repeat(32)
    const args = ['NovelForge.exe', `--nf-release-homepage-smoke=${token}`]
    ...
    expect(parseReleaseOfficialHomepageSmokeInvocation(args, releaseHomepageSmokeEnv({
      NOVELFORGE_RELEASE_HOMEPAGE_SMOKE: '1',
    }))).toBeUndefined()
    expect(parseReleaseOfficialHomepageSmokeInvocation(args, releaseHomepageSmokeEnv({
      NOVELFORGE_RELEASE_HOMEPAGE_SMOKE: '1',
      NOVELFORGE_RELEASE_HOMEPAGE_SMOKE_TOKEN: 'b'.repeat(32),
    }))).toBeUndefined()
    expect(parseReleaseOfficialHomepageSmokeInvocation(args, releaseHomepageSmokeEnv({
      NOVELFORGE_RELEASE_HOMEPAGE_SMOKE: '1',
      NOVELFORGE_RELEASE_HOMEPAGE_SMOKE_TOKEN: token,
    }))).toEqual({ token })
  })

  it('uses the packaged preload bridge to invoke only the fixed no-argument intent and records both shell outcomes', async () => {
    const token = 'a'.repeat(32)
    process.env.NOVELFORGE_RELEASE_HOMEPAGE_SMOKE = '1'
    process.env.NOVELFORGE_RELEASE_HOMEPAGE_SMOKE_TOKEN = token

    let currentHandler: (() => Promise<unknown>) | undefined
    const destroy = vi.fn()
    const executeJavaScript = vi.fn(async (source: string, userGesture?: boolean) => {
      expect(source).toContain("window.novelforgeAPI.invoke('official-homepage:open')")
