...
function temporaryRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'nf-release-skin-smoke-'))
...
function withSmokeEnvironment<T>(root: string, callback: () => T): T {
  const previous = {
    NOVELFORGE_RELEASE_SKIN_SMOKE: process.env.NOVELFORGE_RELEASE_SKIN_SMOKE,
    NOVELFORGE_RELEASE_SKIN_SMOKE_TOKEN: process.env.NOVELFORGE_RELEASE_SKIN_SMOKE_TOKEN,
    NOVELFORGE_HOME_OVERRIDE: process.env.NOVELFORGE_HOME_OVERRIDE,
  }
  process.env.NOVELFORGE_RELEASE_SKIN_SMOKE = '1'
  process.env.NOVELFORGE_RELEASE_SKIN_SMOKE_TOKEN = smokeToken
  process.env.NOVELFORGE_HOME_OVERRIDE = path.join(root, 'isolated-novelforge-home')
  try {
    return callback()
  } finally {
...
describe('packaged skin qualification smoke', () => {
  it('requires a matching one-time command and environment token', () => {
    expect(parseReleaseSkinSmokeInvocation(
      [`--nf-release-skin-smoke=${smokeToken}`],
      {
        NOVELFORGE_RELEASE_SKIN_SMOKE: '1',
        NOVELFORGE_RELEASE_SKIN_SMOKE_TOKEN: smokeToken,
    ...
    expect(parseReleaseSkinSmokeInvocation(
      ...
      {
        NOVELFORGE_RELEASE_SKIN_SMOKE: '1',
        NOVELFORGE_RELEASE_SKIN_SMOKE_TOKEN: 'b'.repeat(64),
      } as unknown as NodeJS.ProcessEnv,
    )).toBeUndefined()
  })
  ...
  it('proves that the file-loaded renderer URL resolves the packaged anime asset and an isolated custom skin survives import, read, and restart', () => {
    ...
    const skinRoot = path.join(root, 'isolated-novelforge-home', 'skins')
