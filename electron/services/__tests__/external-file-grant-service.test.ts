...
describe('ExternalFileGrantService', () => {
  ...
  it('拒绝把绝对路径伪装成目录授权的相对路径', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-external-grant-root-'))
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-external-grant-outside-'))
    const grants = new ExternalFileGrantService({
      now: () => 1_000,
  ...
  it('只签发 root-relative capability；reparse 拒绝交给句柄绑定执行器', () => {
    const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-external-grant-link-'))
  ...
  it('精确文件授权只能解析用户明确选择的那一个文件', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-external-grant-file-'))
  ...
  it('有限次数授权用尽后不能被继续重放', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-external-grant-uses-'))
  ...
  it('窗口销毁时撤销该窗口的全部内存授权', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-external-grant-revoke-'))
    fs.writeFileSync(path.join(root, 'chapter.txt'), 'chapter', 'utf8')
    const grants = new ExternalFileGrantService({
      now: () => 1_000,
  ...
  it('主进程可在消费前复核边界，最后一次消费会删除授权记录', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-external-grant-revalidate-'))
    const grants = new ExternalFileGrantService({
      now: () => 1_000,
