...
import { NOVELFORGE_HOME } from '../utils/config-utils'
  ...
  try {
    ...
    try {
      ...
      if ((error as NodeJS.ErrnoException).code !== 'ENOENT') {
        console.warn('[NovelForge Skin] Unable to remove temporary skin file.', error)
      }
    }
  }
}
...
export class SkinService {
  ...
  constructor(options: SkinServiceOptions = {}) {
    this.rootDirectory = options.rootDirectory ?? path.join(NOVELFORGE_HOME, 'skins')
  ...
  private cleanupAsset(assetFile: string): void {
    ...
    try {
      ...
      if ((error as NodeJS.ErrnoException).code !== 'ENOENT') {
        console.warn('[NovelForge Skin] Unable to clean up a previous skin asset.', error)
      }
    }
  }
