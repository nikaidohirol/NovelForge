import { ensureNovelForgeHome, NOVELFORGE_HOME } from './utils/config-utils'
...
export function registerIPCHandlers() {
  // 确保全局配置目录结构存在
  ensureNovelForgeHome()

  // 皮肤存储损坏或不可用时必须降级为经典皮肤，不能阻断其余 IPC 注册。
  try {
    skinService.initialize()
  } catch (error) {
    console.warn('[NovelForge Skin] 皮肤服务初始化失败，已降级并继续启动应用。', error)
  }
  ...
  console.log(`[NovelForge IPC] 所有 Controller 已注册完成 | 全局工作区: ${NOVELFORGE_HOME}`)
}
