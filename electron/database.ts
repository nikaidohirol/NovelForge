...
export function initProjectDatabase(projectPath: string, importSourceSecret?: Buffer): void {
  ...
  const dbPath = path.join(projectPath, '.novelforge', 'novelforge.db')
  ...
  console.log(`[NovelForge DB] 项目数据库已打开: ${dbPath}`)
}
...
function createTables(db: BetterSqlite3.Database, importSourceSecret?: Buffer) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS characters (
      ...
      anime_json TEXT NOT NULL DEFAULT '',        -- 二次元扩展（色板/萌属性/口头禅/目标/秘密/属性面板）；空 = 无扩展
      created_at TEXT DEFAULT (datetime('now')),
      updated_at TEXT DEFAULT (datetime('now'))
    );
  ...
  if (legacyIdentityTable) {
    ...
    if (legacy) {
      ...
      cipher.setAAD(Buffer.from('novelforge:legacy-import-source-salt:v1', 'utf8'))
  ...
  // 二次元角色卡扩展列：早期数据库 CREATE TABLE IF NOT EXISTS 不会补列。
  const animeCharacterColumns = db.prepare('PRAGMA table_info(characters)').all() as Array<{ name: string }>
  if (!animeCharacterColumns.some(column => column.name === 'anime_json')) {
    db.exec("ALTER TABLE characters ADD COLUMN anime_json TEXT NOT NULL DEFAULT ''")
  }
