...
describe('summary continuity migration', () => {
  it('adds finalized continuity columns after opening a legacy character snapshot table', () => {
    const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-summary-migration-'))
  ...
  it('cascades a migrated finalized continuity snapshot when its draft is deleted', () => {
    const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-summary-cascade-migration-'))
    roots.push(projectRoot)
    const novelforgeRoot = path.join(projectRoot, '.novelforge')
    fs.mkdirSync(novelforgeRoot, { recursive: true })
    const legacyDb = new Database(path.join(novelforgeRoot, 'novelforge.db'))
