...
describe('知识库向量维度', () => {
  ...
  it('closeConnection 会真正释放缓存的 LanceDB 原生连接', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-close-'))
  ...
  it('接受嵌入模型返回的非 2048 维有限向量', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-dimension-'))
    projects.push(projectPath)
  ...
  it('2048 维控制组能够写入同一个公共接口', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-control-'))
    projects.push(projectPath)
  ...
  it('FTS-only 文档不依赖向量列，仍可导入和检索', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-fts-only-'))
    projects.push(projectPath)
  ...
  it('替换同名文档时清理旧的文档元数据', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-superseded-document-'))
    projects.push(projectPath)
  ...
  it('删除文档时按驼峰 docId 移除其 chunks', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-remove-document-'))
    projects.push(projectPath)
  ...
  it('重开后能发现并清理未登记代际中的 embedding-only 半写文档', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-embedding-only-crash-'))
    projects.push(projectPath)
  ...
  it.each([
    ...
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-generation-index-'))
    projects.push(projectPath)
    const vector = [0.1, 0.2, 0.3, 0.4]
  ...
  it('removeDocument 在任一物理代际仍有残留时返回 false，重试后达到全代际零行', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-remove-postcondition-'))
    projects.push(projectPath)
    const vector = [0.1, 0.2, 0.3, 0.4]
  ...
  it('任一非活跃 embedding generation 存在缺块时仍判定文档不完整', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-all-generations-'))
    projects.push(projectPath)
  ...
  it('在大小写敏感的 LanceDB SQL 上按章节范围检索', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-chapter-scope-'))
    projects.push(projectPath)
  ...
  it('在大小写敏感的 LanceDB SQL 上将向量检索限定在章节范围', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-chapter-scope-vector-'))
    projects.push(projectPath)
  ...
  it('安全登记缺少元数据的旧 2048 维 chunks 表并保持可检索', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-legacy-'))
    projects.push(projectPath)
  ...
  it('旧 vectors.json 按真实 1536 维迁移，而不是回退到固定 2048', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-migration-'))
    projects.push(projectPath)
    const vector = Array.from({ length: 1536 }, (_, index) => index / 1536)
    const novelforgePath = path.join(projectPath, '.novelforge')
    fs.mkdirSync(novelforgePath, { recursive: true })
    const sourcePath = path.join(novelforgePath, 'vectors.json')
  ...
  it('迁移遇到不兼容空间时保留原 JSON、旧 active 和旧表', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-migration-failure-'))
    projects.push(projectPath)
    ...
    const sourcePath = path.join(projectPath, '.novelforge', 'vectors.json')
    fs.writeFileSync(sourcePath, JSON.stringify({
      documents: [],
      entries: [{
        id: 'new-chunk',
  ...
  it('迁移预检发现不兼容的旧 chunks 表时不创建新的嵌入空间注册表', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-migration-preflight-'))
    projects.push(projectPath)
    const db = await getConnection(projectPath)
    ...
    const novelforgePath = path.join(projectPath, '.novelforge')
    const registryPath = path.join(novelforgePath, 'embedding-spaces.json')
    const sourcePath = path.join(novelforgePath, 'vectors.json')
    fs.writeFileSync(sourcePath, JSON.stringify({
  ...
  it('两份旧 JSON 中后一份非法时不写入前一份，重试与修正后都不会重复迁移', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-migration-atomic-'))
    projects.push(projectPath)
    const novelforgePath = path.join(projectPath, '.novelforge')
    fs.mkdirSync(novelforgePath, { recursive: true })
    const sourcePath = path.join(novelforgePath, 'vectors.json')
  ...
  it('迁移写入后若无法标记原 JSON，会回滚全部文档且修正环境后只迁移一次', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-migration-marker-'))
    projects.push(projectPath)
    const novelforgePath = path.join(projectPath, '.novelforge')
    fs.mkdirSync(novelforgePath, { recursive: true })
    const sourcePath = path.join(novelforgePath, 'vectors.json')
    ...
    expect(fs.existsSync(sourcePath)).toBe(true)
    expect(fs.existsSync(`${sourcePath}.migrated`)).toBe(false)
    expect(fs.existsSync(path.join(novelforgePath, 'vectors.json.migration-journal.json'))).toBe(false)
  ...
  it('迁移已提交但进程在删除 journal 前退出时，会用已标记源文件安全完成恢复', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-migration-commit-recovery-'))
    projects.push(projectPath)
    const novelforgePath = path.join(projectPath, '.novelforge')
    fs.mkdirSync(novelforgePath, { recursive: true })
    const sourcePath = path.join(novelforgePath, 'vectors.json')
    ...
    const journalPath = path.join(novelforgePath, 'vectors.json.migration-journal.json')
  ...
  it.each([
    ...
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-invalid-'))
    projects.push(projectPath)
  ...
  it('模型或维度变化要求显式重建，并在拒绝前保持旧代际 active', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-generation-'))
    projects.push(projectPath)
  ...
  it('同一维度但不同模型身份同样要求显式重建，不会混写同一表', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-model-conflict-'))
    projects.push(projectPath)
  ...
  it('显式回填完整验证新空间后才切换 active，旧表仍保留', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-rebuild-'))
    projects.push(projectPath)
  ...
  it('候选空间含孤儿或重复块时不切换 active', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-incomplete-candidate-'))
    projects.push(projectPath)
    ...
    fs.writeFileSync(path.join(projectPath, '.novelforge', 'embedding-spaces.json'), `${JSON.stringify({
      ...before,
  ...
  it('catalog 切换落盘失败时补偿本次 docId，旧表、registry、docs 与 canonical 不变', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-rollback-'))
    projects.push(projectPath)
  ...
  it.each([
    ...
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-orphan-first-import-'))
    projects.push(projectPath)
    const db = await getConnection(projectPath)
    ...
    expect(fs.existsSync(path.join(projectPath, '.novelforge', 'embedding-spaces.json'))).toBe(false)
  ...
  it('createTable 内部创建代际后失败时清理本次表并让重试复用同一 generation', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-create-owned-'))
    projects.push(projectPath)
    const db = await getConnection(projectPath)
  ...
  it('安全复用未登记的空物理代际目录，不轮换 generation', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-empty-orphan-'))
    projects.push(projectPath)
    const orphanDirectory = path.join(projectPath, '.novelforge', 'lancedb', 'chunks__space_1.lance')
  ...
  it('保留未登记的非空物理代际目录并在重试时拒绝覆盖而非继续轮换', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-foreign-orphan-'))
    projects.push(projectPath)
    const orphanDirectory = path.join(projectPath, '.novelforge', 'lancedb', 'chunks__space_1.lance')
    ...
    expect(fs.existsSync(path.join(projectPath, '.novelforge', 'lancedb', 'chunks__space_2.lance'))).toBe(false)
  })

  it('首次导入 registry 原子写重复失败不堆积孤儿表或跳 generation', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-orphan-registry-'))
    projects.push(projectPath)
    const db = await getConnection(projectPath)
  ...
  it('updateChunkVectors 新代际 registry 写入重复失败时删除新表并保留旧 active', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-orphan-update-registry-'))
    projects.push(projectPath)
  ...
  it('updateChunkVectors 最终验证失败时删除本次新表且重试仍使用同一 generation', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-orphan-update-verify-'))
    projects.push(projectPath)
  ...
  it('updateChunkVectors 对预存 building 空间失败时只回滚本次行，不删除物理表或旧 active', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-vector-orphan-update-existing-'))
    projects.push(projectPath)
    ...
    fs.writeFileSync(
      path.join(projectPath, '.novelforge', 'embedding-spaces.json'),
