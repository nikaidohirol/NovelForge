...
function makeLegacyProject(): string {
  const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-legacy-finalization-'))
...
describe('finalization outbox migration', () => {
  ...
  it('does not rewrite a legitimate empty snapshot when a current-schema database reopens', () => {
    const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-current-finalization-'))
    roots.push(projectRoot)
    initProjectDatabase(projectRoot)
