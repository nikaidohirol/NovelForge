...
describe('知识库嵌入空间回填', () => {
  ...
  it('通过主进程知识库生产缝逐字节保存并读回混合 UTF-8 参照文本', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-kb-utf8-roundtrip-'))
  ...
  it('imports planning material as FTS-only without calling an embedding provider', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-kb-planning-local-'))
    projects.push(projectPath)
  ...
  it('keeps the first finalized chapter FTS-only when local planning material already awaits an explicit rebuild', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-kb-planning-then-finalize-'))
    projects.push(projectPath)
    const model = {
      baseUrl: 'https://embedding.example/v1',
      apiKey: 'configured-key',
  ...
  it('写作检索的 FTS 与语义路径都按 corpus kind 排除参照语料', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-kb-writing-corpus-'))
    projects.push(projectPath)
  ...
  it('ranks every lexical match before topK so a late explicit planning hint is retained', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-kb-planning-ranking-'))
    projects.push(projectPath)
    const model = {
      baseUrl: 'https://embedding.example/v1',
      apiKey: 'test-key-not-persisted',
  ...
  it('回填按模型实际 1536 维建空间并激活，不重建 chunks 全文表', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-kb-backfill-'))
    projects.push(projectPath)
  ...
  it('同一模型指纹从满 768 维空间变为 1536 维时，探测实际响应并完整建立新代际', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-kb-dimension-rebuild-'))
    projects.push(projectPath)
    const identity = {
  ...
  it('同一模型指纹变更维度的重建失败时，旧 active 代际、表和查询结果保持不变', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-kb-dimension-rebuild-failure-'))
    projects.push(projectPath)
    const identity = {
      modelFingerprint: 'openai|https://embedding.example/v1|same-model',
      distanceMetric: 'l2',
  ...
  it('回填响应包含非法数值时在 Arrow 前失败，全文表和 FTS 保持可用', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-kb-backfill-invalid-'))
    projects.push(projectPath)
  ...
  it('导入遇到无效 Embedding 响应时返回可理解错误且不打开 LanceDB', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-kb-import-invalid-response-'))
    projects.push(projectPath)
    ...
    expect(fs.existsSync(path.join(projectPath, '.novelforge', 'lancedb'))).toBe(false)
  })

  it('文件导入遇到无效 Embedding 响应时不打开 LanceDB', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-kb-file-invalid-response-'))
    projects.push(projectPath)
  ...
  it('importText 遇到 reindex_required 时不先删除同名旧文档', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-kb-import-conflict-'))
    projects.push(projectPath)
    const oldSpace = {
  ...
  it('嵌入空间元数据损坏时回填与缺失统计显式失败，不伪报 0 条成功', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-kb-corrupt-registry-'))
    projects.push(projectPath)
    expect(await addChunks(projectPath, 'fts-document', 'fts.txt', ['无法静默吞掉的全文正文']))
      .toEqual({ success: true, chunkCount: 1 })
    fs.writeFileSync(path.join(projectPath, '.novelforge', 'embedding-spaces.json'), '{invalid json', 'utf8')
  ...
  it('旧 vectors.json 迁移失败时阻断导入、回填和检索，修正前不创建新空间', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-kb-migration-barrier-'))
    projects.push(projectPath)
    ...
    fs.writeFileSync(path.join(novelforgePath, 'vectors.json'), JSON.stringify({
      documents: [
  ...
  it('active 代际后 FTS-only 增量导入会先撤销 active，搜索仍能命中新文档并且全量重建后才重新激活', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-kb-fts-increment-invalidate-'))
    projects.push(projectPath)
    const space = {
  ...
  it('FTS-only 增量后回填失败保持 no-active，旧向量表不被破坏且 FTS 仍包含新增文档', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-kb-fts-increment-failure-'))
    projects.push(projectPath)
    const space = {
      modelFingerprint: 'openai|https://embedding.example/v1|model-768',
      distanceMetric: 'l2',
  ...
  it('无法先持久化 active 降级时拒绝写入 FTS-only canonical，旧代际保持完整', async () => {
    const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-kb-fts-increment-registry-failure-'))
    projects.push(projectPath)
    const space = {
      modelFingerprint: 'openai|https://embedding.example/v1|model-768',
      distanceMetric: 'l2',
