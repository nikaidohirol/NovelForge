...
describe('project writing language persistence', () => {
  it('round-trips mixed UTF-8 project facts byte-for-byte across close and reopen', () => {
    const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-writing-language-utf8-'))
  ...
  it('initializes and reopens a new project with its selected writing language', () => {
    const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-writing-language-new-'))
    roots.push(projectRoot)
  ...
  it('keeps a legacy project in Chinese and restores an explicitly saved language', () => {
    const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'novelforge-writing-language-'))
    roots.push(projectRoot)
    const novelforgeRoot = path.join(projectRoot, '.novelforge')
    fs.mkdirSync(novelforgeRoot, { recursive: true })
    const legacyDb = new Database(path.join(novelforgeRoot, 'novelforge.db'))
    legacyDb.exec(`
