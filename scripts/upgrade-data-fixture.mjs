...
const PROJECT_MANIFEST_RELATIVE_PATH = '.novelforge/project.json'
const PROMPT_TEMPLATE_RELATIVE_PATH = '.novelforge/prompts/chapter-style.md'
const FINALIZED_MANUSCRIPT_RELATIVE_PATH = '第7章 失真的航标.txt'
const EMBEDDING_REGISTRY_RELATIVE_PATH = '.novelforge/embedding-spaces.json'
const LANCEDB_RELATIVE_PATH = '.novelforge/lancedb'
const ASSET_INVENTORY_RELATIVE_PATH = '.novelforge/upgrade-data-inventory.json'
...
function databasePath(projectRoot) {
  return join(resolve(projectRoot), '.novelforge', 'novelforge.db')
}
...
async function seed(projectRoot, settingsPath) {
  ...
  mkdirSync(join(resolve(projectRoot), '.novelforge'), { recursive: true })
