...
export const RENDERER_SURFACE_E2E_CONTRACT = Object.freeze({
  smokeEnvironment: Object.freeze([
    'NOVELFORGE_HOME_OVERRIDE',
    'NOVELFORGE_SMOKE_OPEN_PROJECT',
    'NOVELFORGE_SMOKE_PROJECT_MARKER',
  ]),
  ...
  visualEvidence: Object.freeze({
    outputEnvironment: 'NOVELFORGE_RENDERER_VISUAL_EVIDENCE_DIR',
...
function createIsolatedFixture() {
  ...
  const novelforgeHome = join(temporaryRoot, 'novelforge-home')
  const electronUserData = join(temporaryRoot, 'electron-user-data')
  const markerPath = join(temporaryRoot, 'project-opened.json')
  const manifestRoot = join(projectRoot, '.novelforge')
  mkdirSync(manifestRoot, { recursive: true })
  mkdirSync(novelforgeHome, { recursive: true })
  ...
  const skinRoot = join(novelforgeHome, 'skins')
  ...
  return { temporaryRoot, projectRoot, novelforgeHome, electronUserData, markerPath }
}
...
async function launchIsolatedElectron(fixture, captureDiagnostic) {
  ...
  environment.NOVELFORGE_HOME_OVERRIDE = fixture.novelforgeHome
  environment.NOVELFORGE_SMOKE_OPEN_PROJECT = fixture.projectRoot
  environment.NOVELFORGE_SMOKE_PROJECT_MARKER = fixture.markerPath
