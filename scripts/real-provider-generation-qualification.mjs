...
const QUALIFICATION_TARGETS = Object.freeze([
  ...
  Object.freeze({
    ...
    protocol: 'gemini',
    modelName: 'gemini-2.5-flash-lite',
    baseUrl: 'https://generatinovelforgenguage.googleapis.com',
  }),
])
...
const GEMINI_KEY_ENV = 'NOVELFORGE_QUALIFICATION_GEMINI_API_KEY'
...
function configuredProfilesFromDisk(environment) {
  const novelforgeHome = String(environment.NOVELFORGE_HOME_OVERRIDE || '').trim()
    || path.join(homedir(), '.novelforge')
  const parsed = JSON.parse(readFileSync(path.join(novelforgeHome, 'models.json'), 'utf8'))
...
async function runCli() {
  ...
  if (options.mode === 'execute') {
    allowBillableRequests = process.env.NOVELFORGE_REAL_PROVIDER_QUALIFICATION === BILLABLE_CONFIRMATION
