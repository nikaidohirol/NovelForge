...
const expression = `(async () => {
  if (!window.novelforgeAPI || typeof window.novelforgeAPI.invoke !== 'function') {
    throw new Error('legacy preload API is unavailable')
  }
  const result = await window.novelforgeAPI.invoke('project:open', ${JSON.stringify(resolve(projectPath))})
