...
describe('ArchFileViewer locale', () => {
  it.each([
    ...
    await act(async () => root.render(
      <ArchFileViewer tabId={`arch-${key}`} filePath={`novelforge://core/${key}`}
  ...
  it('reports a rejected save without clearing the draft and rejects exit-save', async () => {
    ...
    useEditorStore.setState({ tabs: [{
      ...
      projectKey: PROJECT_PATH, filePath: 'novelforge://core/premise',
      content: '未保存的故事前提', savedContent: '', dirty: true,
    }], draftLedgers: {} })
    await act(async () => root.render(
      <ArchFileViewer tabId="arch-save-error" filePath="novelforge://core/premise"
  ...
  it('keeps its exit-save handler while an inactive tab is unmounted', async () => {
    ...
    await act(async () => root.render(
      <ArchFileViewer tabId="arch-inactive" filePath="novelforge://core/premise"
