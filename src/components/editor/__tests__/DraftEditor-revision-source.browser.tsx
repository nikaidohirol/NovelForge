...
describe('DraftEditor revision source binding', () => {
  ...
  it('saves each dirty draft through its own retained handler after switching the active tab', async () => {
    ...
    const tabs = [
      {
        ...
        filePath: 'novelforge://draft/7', content: firstContent, savedContent: SOURCE,
      ...
      },
      {
        ...
        filePath: 'novelforge://draft/8', content: secondContent, savedContent: '第二章旧稿。',
    ...
    await act(async () => root.render(
      <DraftEditor key="draft-7" tabId="draft-7" filePath="novelforge://draft/7" content={firstContent} projectKey={PROJECT_PATH} />,
    ))
    useEditorStore.getState().setActiveTab('draft-8')
    await act(async () => root.render(
      <DraftEditor key="draft-8" tabId="draft-8" filePath="novelforge://draft/8" content={secondContent} projectKey={PROJECT_PATH} />,
    ))
