...
function DraftEditorSession({ tabId, filePath, content, projectKey }: Props) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const chapterDir = `novelforge://draft/ch${m.chapterNumber}`
      // 检查待合并修稿
  ...
  /** 保存（novelforge://draft/ 走 DB，其他走 FS） */
  ...
  const openPendingRevision = async (rev: RevisionEntry) => {
    ...
    // 使用 novelforge://revision/{id} 协议路径读取修稿内容
    const revPath = `novelforge://revision/${rev.id}`
  ...
  const handleMergeComplete = async (mergedText: string) => {
    ...
    const chapterDir = `novelforge://draft/ch${meta.chapterNumber}`
    const currentTarget = useEditorStore.getState().tabs.find(
      tab => tab.id === tabId && tab.projectKey === projectKey,
    )
    if (
  ...
  const openLatestReview = async () => {
    ...
    const reportContent = await readDraftBody(`novelforge://review/${latest.id}`, projectKey, projectSession)
    if (!isProjectSessionCurrent(projectSession)) return
