    ...
    >
      ...
      <div className="flex-1 overflow-hidden">
        {activeTab?.type === 'chapter' && activeTab.projectKey === currentProject.path && (
          activeTab.filePath?.startsWith('novelforge://draft/')
          || activeTab.filePath?.startsWith('novelforge://manuscript/')
        ) && (
        ...
        {activeTab?.type === 'chapter' && activeTab.projectKey === currentProject.path
          && activeTab.filePath?.startsWith('novelforge://recovery/') && (
          <ProseEditorWrapper
            key={activeTab.id}
            tab={activeTab}
            onSave={async content => {
              const projectSession = captureProjectSession(currentProject)
              const candidateId = activeTab.filePath?.slice('novelforge://recovery/'.length)
              if (
                !projectSession
                || !candidateId
