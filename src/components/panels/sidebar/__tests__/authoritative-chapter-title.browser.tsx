...
beforeEach(() => {
  ...
  ;(window as unknown as { novelforgeAPI: TestNovelForgeApi }).novelforgeAPI = {
    invoke,
    on: () => () => {},
    once: () => {},
    send: () => {},
...
afterEach(async () => {
  ...
  delete (window as unknown as { novelforgeAPI?: TestNovelForgeApi }).novelforgeAPI
})
...
describe('authoritative finalized chapter titles', () => {
  ...
  it('keeps author outbox titles in both finalized surfaces despite missing or conflicting reference blueprints', async () => {
    ...
    const manuscriptFiles = [chapterOne, chapterTwo].map(item => ({
      path: `novelforge://manuscript/${item.id}`,
