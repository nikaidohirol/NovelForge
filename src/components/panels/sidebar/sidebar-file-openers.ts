...
export async function openArchFile(filePath: string, name: string): Promise<void> {
  ...
  let content = ''
  // 支持 novelforge://core/ 伪协议路径，从 DB 读取架构字段
  if (filePath.startsWith('novelforge://core/')) {
    const { readCoreContent } = await import('../../../services/novelforge-protocol')
    try {
...
export async function openChapterFile(filePath: string, name: string): Promise<void> {
  ...
  try {
    if (filePath.startsWith('novelforge://')) {
      const { readNovelForgeContent } = await import('../../../services/novelforge-protocol')
      content = await readNovelForgeContent(filePath, projectSession)
      if (!isProjectSessionCurrent(projectSession)) return
      const draftIdMatch = filePath.match(/^novelforge:\/\/(?:draft|manuscript)\/(\d+)$/)
      if (draftIdMatch) {
