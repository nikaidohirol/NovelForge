...
export default function ProjectTree() {
  ...
  const manuscriptFiles = Object.values(draftsByChapter)
    ...
    .map(draft => ({
      path: `novelforge://manuscript/${draft!.id}`, // 诸如 novelforge://manuscript/42
      ...
      {open && (
        <div>
          {ARCH_FILES.map(f => {
            ...
            const filePath = `novelforge://core/${f.key}`
            return (
              <ArchFileRow
                key={f.key}
                f={f}
...
function ArchFileRow({
  ...
  const clearArchFile = async () => {
    ...
    const { writeCoreContent } = await import('../../../services/novelforge-protocol')
