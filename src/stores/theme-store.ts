...
export type Theme = 'anime' | 'light' | 'galaxy' | 'paper' | 'dark'
...
interface ThemeState {
  ...
  theme: Theme
  /** 实际应用的主题（解析 system 后） */
  resolvedTheme: 'anime' | 'light' | 'galaxy' | 'paper' | 'dark'
  /** 当前缩放级别（1.0 = 100%） */
  zoom: number
...
function resolveTheme(theme: Theme): 'anime' | 'light' | 'galaxy' | 'paper' | 'dark' {
  if ((theme as string) === 'system') {
    return window.matchMedia('(prefers-color-scheme: dark)').matches ? 'anime' : 'anime'
  }
  return theme
}

/** 应用主题到 DOM — 支持互斥的主题 */
function applyTheme(theme: 'anime' | 'light' | 'galaxy' | 'paper' | 'dark') {
  const root = document.documentElement
  root.classList.remove('galaxy', 'paper', 'dark', 'anime')
  if (theme === 'galaxy') {
