...
describe('editor store project-scoped tabs', () => {
  ...
  it('keeps the same numeric draft id in projects A and B as separate tabs', () => {
    const filePath = 'novelforge://draft/1'
    useEditorStore.getState().openFile({
      id: filePath,
      name: 'A draft',
      type: 'chapter',
  ...
  it('keeps dirty architecture drafts isolated when two projects use the same NovelForge path', () => {
    const filePath = 'novelforge://core/premise'
