...
describe('draft-store merged revision persistence', () => {
  ...
  it('marks the direct novelforge revision URI as merged after its target draft is updated', async () => {
    const result = await useDraftStore.getState().applyMergedRevision(
      'novelforge://draft/ch1',
      1,
      'novelforge://draft/11',
      'novelforge://revision/1',
      '已人工合并的修订稿',
      '原稿',
      projectPath,
      projectSession,
