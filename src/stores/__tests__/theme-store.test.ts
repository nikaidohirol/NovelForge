...
describe('theme store branding defaults', () => {
  ...
  it('defaults to the accepted sakura anime theme', async () => {
    const { useThemeStore } = await import('../theme-store')

    useThemeStore.getState().initTheme()

    expect(useThemeStore.getState().theme).toBe('anime')
    expect(useThemeStore.getState().resolvedTheme).toBe('anime')
  })

  it('uses the anime default when a historical persisted state omits the theme field', async () => {
  ...
  it('fails safely to the anime default when persisted JSON is corrupt', async () => {
    localStorage.setItem('novelforge-theme', '{not-json')
  ...
  it('persists theme state under the AI novel writer storage key', async () => {
    ...
    expect(localStorage.getItem('novelforge-theme')).toContain('"dark"')
  ...
  it('migrates the historical night theme to dark exactly once', async () => {
    ...
    expect(useThemeStore.getState().theme).toBe('dark')
    expect(useThemeStore.getState().resolvedTheme).toBe('dark')
    expect(JSON.parse(localStorage.getItem('novelforge-theme') ?? '{}')).toMatchObject({
      state: { theme: 'dark' },
      version: 1,
    })
    expect(localStorage.getItem('novelforge-theme-migrated')).toBeNull()
  })
  ...
  it('keeps theme initialization idempotent after migration', async () => {
    ...
    useThemeStore.getState().initTheme()
    const firstPersistedState = localStorage.getItem('novelforge-theme')
    useThemeStore.getState().initTheme()

    expect(useThemeStore.getState().theme).toBe('dark')
    expect(useThemeStore.getState().resolvedTheme).toBe('dark')
    expect(localStorage.getItem('novelforge-theme')).toBe(firstPersistedState)
  })
})
