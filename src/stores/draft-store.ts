...
export const useDraftStore = create<DraftState>()((set, get) => ({
  ...
  loadChapterDrafts: async (chapterNumber, expectedProjectPath, expectedProjectSession) => {
    ...
    try {
      ...
      const metas: DraftMeta[] = list.map((m) => ({
        ...
        filePath: `novelforge://draft/${m.id}`
      }))
    ...
    try {
      ...
      for (const draft of drafts) {
        ...
        chapterDrafts.push({
          ...
          filePath: `novelforge://draft/${draft.id}`
        })
  ...
  markDraftStatus: async (draftPath, chapterNumber, status, expectedProjectSession) => {
    ...
    if (!project || !projectSession) return
    const directDraftId = /^novelforge:\/(?:draft|manuscript)\/(\d+)$/.exec(draftPath)?.[1]
  ...
  applyMergedRevision: async (
    ...
    try {
      ...
      const directRevisionId = /^novelforge:\/\/revision\/(\d+)$/.exec(revPath)?.[1]
...
/**
 * 读取草稿文件正文（委托给 novelforge-protocol 统一路由）
 * @deprecated 建议直接使用 readNovelForgeContent()
 */
export async function readDraftBody(
  ...
  if (filePath.startsWith('novelforge://revision/')) {
    const full = await ipc.invokeWithProjectSession(
      ...
      parseInt(filePath.replace('novelforge://revision/', '')),
      expectedProjectPath,
    )
    if (!isDraftProjectSessionCurrent(projectSession)) return ''
    return full?.content ?? ''
  ...
  if (filePath.startsWith('novelforge://review/')) {
    const full = await ipc.invokeWithProjectSession(
      projectSession,
      'db:review-get-full',
      parseInt(filePath.replace('novelforge://review/', '')),
      expectedProjectPath,
    )
    if (!isDraftProjectSessionCurrent(projectSession)) return ''
    return full?.content ?? ''
  ...
  if (filePath.startsWith('novelforge://core/')) {
    ...
    if (!isDraftProjectSessionCurrent(projectSession)) return ''
    const key = filePath.replace('novelforge://core/', '')
