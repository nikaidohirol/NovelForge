...
export const ipcPromptPersistence: PromptPersistence = {
  ...
  async loadProject(projectSession) {
    const dirPath = `${projectSession.projectPath}/.novelforge/prompts`
  ...
  async saveProject(projectSession, template) {
    ...
    if (!exists) {
      requireIpcSuccess(await ipc.invokeWithProjectSession(
        projectSession,
        'fs:mkdir',
        `${projectSession.projectPath}/.novelforge`,
        projectSession.projectPath,
      ), '创建项目配置目录')
      requireIpcSuccess(await ipc.invokeWithProjectSession(
        projectSession,
  ...
  async deleteProject(projectSession, key, writingLanguage = 'zh-CN') {
    const language = resolveWritingLanguage(writingLanguage)
    const filePaths = [`${projectSession.projectPath}/.novelforge/prompts/${key}.${language}.json`]
    if (language === 'zh-CN') filePaths.push(`${projectSession.projectPath}/.novelforge/prompts/${key}.json`)
