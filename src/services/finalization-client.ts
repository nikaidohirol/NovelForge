...
interface NovelForgeInvokeApi {
  invoke(channel: string, ...args: unknown[]): Promise<unknown>
}

function getNovelForgeApi(): NovelForgeInvokeApi {
  const api = (window as unknown as { novelforgeAPI?: NovelForgeInvokeApi }).novelforgeAPI
...
export async function commitFinalizationSnapshot(
  ...
  return getNovelForgeApi().invoke(
    'finalization:commit',
    snapshot,
