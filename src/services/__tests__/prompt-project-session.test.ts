...
describe('project custom prompt session ownership', () => {
  it.each([
    ...
    const legacyPath = `${sessionA.projectPath}/.novelforge/prompts/first_chapter_draft.json`
    const localizedPath = `${sessionA.projectPath}/.novelforge/prompts/first_chapter_draft.${writingLanguage}.json`
    ...
    vi.mocked(ipc.invokeWithProjectSession).mockImplementation((async (
      ...
      if (channel === 'fs:check-exists') {
        ...
        return target.endsWith('/.novelforge/prompts') || files.has(target)
      }
    ...
    vi.mocked(ipc.invokeWithProjectSession).mockImplementation((async (
      ...
      if (channel === 'fs:list-dir') {
        return [{
          ...
          path: `${session.projectPath}/.novelforge/prompts/first_chapter_draft.json`,
          isDir: false,
          size: 1,
          modifiedAt: '',
        }]
  ...
  it('isolates one damaged project prompt while publishing the remaining valid overrides', async () => {
      ...
      if (channel === 'fs:list-dir') {
        return [
          ...
          },
          {
            ...
            path: `${session.projectPath}/.novelforge/prompts/next_chapter_draft.json`,
            isDir: false,
            size: 1,
            modifiedAt: '',
          },
    ...
    expect(ipc.invokeWithProjectSession).toHaveBeenCalledWith(
      sessionA,
      'fs:check-exists',
      `${sessionA.projectPath}/.novelforge/prompts`,
