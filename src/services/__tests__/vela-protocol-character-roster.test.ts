...
import { readCoreContent, writeCoreContent } from '../novelforge-protocol'
...
describe('novelforge core character roster seam', () => {
  beforeEach(() => {
  ...
  it('reads the ready deterministic roster projection instead of project-core charactersArch', async () => {
    ...
    await expect(readCoreContent('novelforge://core/characters', projectSession))
      .resolves.toBe('# 角色图谱\n\n## 主角：陆舟')
  ...
  it('rejects direct character-projection writes without reaching project-core IPC', async () => {
    await expect(writeCoreContent('novelforge://core/characters', '不能直接覆盖', projectSession))
      .resolves.toBe(false)
