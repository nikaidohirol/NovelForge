...
export async function parseDraftMeta(
  ...
  // 优先处理 novelforge://draft/{id} 纯数字 ID 格式（DB 化后的标准路径）
  const idMatch = filePath.match(/^novelforge:\/\/(?:draft|manuscript)\/(\d+)$/)
  if (idMatch) {
    ...
    return {
      ...
      filePath: `novelforge://draft/${dbMeta.id}`,
    } as unknown as DraftMeta
  }

  // 兼容旧格式 draft_v(\d+).md 和 novelforge://draft/ch{N}/v{V}
