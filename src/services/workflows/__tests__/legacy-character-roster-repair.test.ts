...
function installNovelForge(invoke: (channel: string, ...args: unknown[]) => unknown) {
  const lease = {
  ...
  vi.stubGlobal('window', {
    novelforgeAPI: {
      invoke: vi.fn((channel: string, ...args: unknown[]) => {
        ...
        if (channel === 'fs:list-dir' && args[0] === `${projectPath}/.novelforge/skills`) {
