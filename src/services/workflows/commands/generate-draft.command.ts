...
export class GenerateDraftCommand extends BaseWorkflowCommand {
  ...
  async execute({ step, context, callbacks }: CommandExecuteParams): Promise<string> {
    ...
    try {
      ...
      const pseudoPath = createResult.id ? `novelforge://draft/${createResult.id}` : `novelforge://draft/ch${this.chapterInfo.chapterNumber}/v${nextVersion}`
  ...
  private async readCharacterProfiles(
    ...
    try {
      ...
      for (const card of roster.entries) {
        ...
        // 二次元角色卡扩展：萌属性/属性面板/口头禅/目标为公开档案，
        // 秘密仅作为叙述层约束注入——叙述者知情，但正文不得提前泄露。
        const anime = card.anime
        if (anime) {
          if (anime.traits.length > 0) facts.push(`moe_traits: ${anime.traits.join('、')}`)
          const panel = [
            anime.panel.hair && `${promptLanguageText(writingLanguage, '发色', 'hair')} ${anime.panel.hair}`,
            anime.panel.eyes && `${promptLanguageText(writingLanguage, '瞳色', 'eyes')} ${anime.panel.eyes}`,
            anime.panel.voice && `${promptLanguageText(writingLanguage, '声线', 'voice')} ${anime.panel.voice}`,
          ].filter(Boolean)
          if (panel.length > 0) facts.push(`panel: ${panel.join(' / ')}`)
          if (anime.catchphrase) facts.push(`catchphrase: ${anime.catchphrase}`)
          for (const goal of anime.goals) facts.push(`goal: ${goal}`)
          for (const secret of anime.secrets) {
            facts.push(promptLanguageText(
              writingLanguage,
              `secret（须保守：在知情角色揭示前不得向其他角色透露）: ${secret}`,
              `secret (must keep: do not reveal to other characters before the knowing character reveals it): ${secret}`,
            ))
          }
        }
