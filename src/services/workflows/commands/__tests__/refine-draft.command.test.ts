  ...
  it('uses the frozen English UI locale for visible refinement logs and the diff tab independently of Chinese writing', async () => {
    ...
    expect(useEditorStore.getState().tabs).toEqual([
      expect.objectContaining({
        ...
        type: 'diff',
        revisionPath: 'novelforge://revision/9',
      }),
    ])
  })
...
describe('ReviewChapterCommand reasoning stage', () => {
  ...
  it('preserves the AI review when deterministic continuity evidence cannot be read', async () => {
    ...
    const persistedReview = JSON.parse(createParams[0]!.content) as {
      summary: string
      items: Array<{ category: string; severity: string; description: string }>
    }
    // 前两条为 AI 审稿输出；其后是本地叠加的目标核对与轻小说感指标条目
    expect(persistedReview.summary).toBe('审稿包含待核实项目，不能视为全部通过。')
    expect(persistedReview.items.slice(0, 2)).toMatchObject([
